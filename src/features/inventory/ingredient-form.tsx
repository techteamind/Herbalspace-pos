...
export function IngredientForm({ initial, onClose }: { initial?: IngredientWithUnit; onClose: () => void }): JSX.Element {
  ...
  async function submit(): Promise<void> {
    if (editing) await update.mutateAsync({ id: initial.id, name, unitId, minStock: parseLocaleNumber(minStock), lastCost: parseLocaleNumber(lastCost) });
    else await create.mutateAsync({ name, unitId, currentStock: parseLocaleNumber(currentStock), minStock: parseLocaleNumber(minStock), lastCost: parseLocaleNumber(lastCost) });
    onClose();
  }
  ...
  return (
    <FormSheet title={editing ? "Edit Bahan Baku" : "Tambah Bahan Baku"} onClose={onClose}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Satuan">
          <select className={inputCls} value={unitId} disabled={editing} onChange={(e) => setUnitId(e.target.value)}>
      ...
      {editing && <p className="font-label-caps text-label-caps text-on-surface-variant">Satuan terkunci setelah bahan dibuat (mengubahnya merusak stok &amp; resep). Stok saat ini diubah lewat penerimaan / penyesuaian, bukan di sini.</p>}
